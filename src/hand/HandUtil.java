...
public class HandUtil {
	...
	public static Hand findHand(ArrayList<Card> playerCards,
	...
	public static Hand getHand(ArrayList<Card> cards) {
		...
		for (Integer j : cardMap.keySet()) {
			if (cardMap.get(j).size() == 4) {
				FourOfAKind f = new FourOfAKind(cardMap.get(j).get(0), cardMap
						.get(j).get(1), cardMap.get(j).get(2), cardMap.get(j)
						.get(3));
				hands.add(f);
				...
				ThreeOfAKind t = new ThreeOfAKind(cardMap.get(j).get(0),
						cardMap.get(j).get(1), cardMap.get(j).get(2));
		...
		if (threeOfAKindList.size() >= 1) {
			...
			if (threeOfAKindList.size() == 2) {
				// Determine what kind of a full house you have given 2
				// ThreeOfAKinds
				fullHouseFound = true;
				...
				if (pairList.size() > 0) {
					...
					if (pairList.size() == 2) {
						...
						if (pairList.get(1).getHighCard().value > currentHighValue) {
							fullHouse = new FullHouse(threeOfAKindList.get(0),
									pairList.get(1));
						} else {
							fullHouse = new FullHouse(threeOfAKindList.get(0),
									pairList.get(0));
		...
		System.out.println("Straights Check");
		...
		System.out.println("Straight Flush Check");
		...
		// *** High Card Only ***
		// ******************************
		System.out.println("High Card Check");
		...
		System.out.println("Finding Highest Hand");
		...
		if (hands.size() == 1) {
			...
			for (int i = 1; i < hands.size(); i++) {
				int compareResult = handComparator.compare(currentHighestHand,
						hands.get(i));

				if (compareResult < 0) {
					currentHighestHand = hands.get(i);
				} else if (compareResult == 0) {
					currentHighestHand = (currentHighestHand.getHighCard().value > hands
							.get(i).getHighCard().value) ? currentHighestHand
							: hands.get(i);
	...
	private static StraightFlush checkStraightFlush(
			ArrayList<Straight> straights) {
	...
	private static void findStraights(ArrayList<Card> cards,
			ArrayList<Straight> straights, int index, int previousValue,
			int sequenceCount, int highestSequenceIndex, boolean straightFound) {
		...
		if (index < cards.size()) {
			...
			if (straightFound) {
				Straight straight = new Straight(
						cards.get(highestSequenceIndex),
			...
			findStraights(cards, straights, index + 1, cards.get(index).value,
					sequenceCount, highestSequenceIndex, straightFound);
		} else {
			return;
		}
	}
